...
export function parseAttributeValueFromValueDto(
  ...
  if (attr.valueDateTime !== undefined) return attr.valueDateTime ?? null;
  if (attr.valueString !== undefined) return attr.valueString ?? null;
  return null;
}
...
export function parseAttributeValuesByKey(
  ...
  const result: Record<string, ParsedAttributeValueEntry> = {};
  if (!attrs || !keyById) return result;
  const keep = options?.keep ?? "last";
  for (const attr of attrs) {
    if (!attr?.attributeId) continue;
...
export function getAttributeValueByKeyFromValues(
  ...
  if (!attrs || !keyById) return fallback;
